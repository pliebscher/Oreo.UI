...
const toDate = new Date().toISOString().substring(0, 10).replaceAll('-', ''); // need to add some days here
...
export async function GET(event: { params: { slug: any; }; }) {
    ...
    const apiUrl = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=predictions&begin_date=${fromDate}&end_date=${toDate}&datum=MLLW&station=${stationId}&time_zone=lst_ldt&units=english&interval=hilo&format=json&application=NOS.COOPS.TAC.TidePred`
